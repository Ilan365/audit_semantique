...
import xlsxwriter
import tldextract  # Ajout de tldextract pour des noms de colonnes propres
...
def read_ahrefs_file(file):
    """Lit les fichiers d'export Ahrefs avec gestion correcte des types."""
    try:
...
def create_competition_audit(combined_data, filters):
    ...
    for keyword, group in grouped:
        ...
        # Nombre total de domaines pour ce mot-clé
        total_domains = group['domain'].nunique()
        ...
        if filter_type == "Au moins 1 site positionné dans le top 10":
            ...
            meets_criteria = meets_criteria and (total_domains >= min_sites)
        ...
        # Volume de recherche (prendre le maximum)
            ...
            row['Nbre de NDD Positionnés'] = total_domains
...
def to_excel(df_dict):
    ...
    for sheet_name, df in df_dict.items():
        ...
        if sheet_name == "Compétition":
            # Dans la fonction to_excel(), remplacez la partie qui gère les noms de colonnes par ceci:
